...
class RollingBackwardNumberSplit(BaseModel):
    """
    Create a `BaseModel` instance from a number of backward rolling ranges of 
    the same length.
    ...
    def __init__(
        ...
        index: range,
        n: int,
        length: tp.Optional[str | int | float | pd.Timedelta] = None,
        split: tp.Optional[int | float | slice | BaseTool | BasePeriod] = None,
        allow_zero_len: tp.Optional[bool] = False,
        range_format: tp.Optional[str] = None,
        freq: tp.Optional[str | int | float | Offset | pd.Timedelta] = "auto",

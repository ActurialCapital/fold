...
from fold.model_selection.models.rolling_split import (
    RollingSplit,
    RollingBackwardSplit
)
...
class RollingBackwardOptimizeSplit(RollingBackwardSplit):
    """
    Create backward rolling ranges of the same optimized length.
    ...
    freq : str | int | float | Offset | pd.Timedelta, optional
        Frequency of the index, by default "auto".
    sort : bool, optional
        If True, sort splitted data, by default False.
    constraints : BaseTool, optional
        Additional constraints, by default None.
    split_labels : range, optional
        Labels for the split, by default None.
    ...
    -----
    This class searches for a range length that covers most of the index to 
    create `n` backward rolling ranges of the same length. The `optimize_anchor_set`
    parameter specifies the index of a set within the backward rolling ranges 
    that should become non-overlapping.
    ...
    def __init__(
        ...
        if split is not None and not isinstance(split, (float, np.floating)):
            raise TypeError(
                "Split must be a float when length='optimize'"
            )
